...
from .models import (
    ...
    FoodDonationImage, IDVerificationImage, MoneyDonation, ContactMessage, MarketplaceBid, MarketplaceChat, Notification, Bhandara, VisitorLog
)
...
from .forms import BhandaraForm
...
def money_donate(request):
    """View for the money donation page showing bank details and QR code"""
    # No form submission needed, just display the page with payment info
...
def custom_admin_dashboard(request):
    ...
    # Get pending and completed donation counts
...
def money_donation_history(request):
    ...
    # Debug message to verify the donations are being found
    print(f"Found {len(user_donations)} donations for user {request.user.username}")
    
    # Include additional information in the context for debugging
    context = {
    ...
    print(f"Rendering money_donation_history.html with {len(user_donations)} donations")
    
    # Add a flash message to confirm the page is being rendered
    messages.info(request, f"Showing {len(user_donations)} donation(s)")
...
def admin_dashboard(request):
    ...
    if request.method == 'POST':
        if 'clear_stats' in request.POST:
            # Clear some statistics or caches if needed
            messages.success(request, 'Statistics cleared successfully.')
    
    # Get statistics
    total_users = User.objects.count()
    total_donations = FoodDonation.objects.count()
    total_volunteers = Volunteer.objects.count()
    total_delivery_assignments = DeliveryAssignment.objects.count()
    completed_deliveries = DeliveryAssignment.objects.filter(status='delivered').count()
    pending_deliveries = DeliveryAssignment.objects.filter(status='assigned').count()
    ...
    recent_donations = FoodDonation.objects.order_by('-created_at')[:5]
    recent_deliveries = DeliveryAssignment.objects.order_by('-created_at')[:5]
    recent_users = User.objects.order_by('-date_joined')[:5]
    
    # Marketplace stats
    total_marketplace_items = MarketplaceItem.objects.count()
    active_marketplace_items = MarketplaceItem.objects.filter(status='available').count()
    pending_approval_listers = MarketplaceLister.objects.filter(status='pending').count()
    
    # Money donations - get all donations
    money_donations = MoneyDonation.objects.all().order_by('-created_at')
    total_money_donated = money_donations.aggregate(total=Sum('amount'))['total'] or 0
    
    # Contact messages
    unread_contact_messages = ContactMessage.objects.filter(status='new').count()
    
    # Get visitor statistics
    visitor_stats = self.get_visitor_stats()
    
    # Get all donations for display
    donations = FoodDonation.objects.all().order_by('-created_at')
    assignments = DeliveryAssignment.objects.all().order_by('-created_at')
    
    # Get marketplace listers for the tab
    marketplace_listers = MarketplaceLister.objects.filter(status='pending').order_by('-created_at')
    
    # Get donation status choices for dropdowns
    donation_status_choices = FoodDonation.STATUS_CHOICES
    assignment_status_choices = DeliveryAssignment.STATUS_CHOICES
    ...
    context = {
        'stats': {
            ...
            'pending_donations': pending_donations,
            'completed_donations': completed_donations,
            'active_assignments': pending_deliveries,
            'cancelled_donations': cancelled_donations,
        },
        'money_donations': money_donations,
...
def get_visitor_stats(self):
    ...
    visitors_today = VisitorLog.objects.filter(visit_timestamp__date=today).count()
    visitors_yesterday = VisitorLog.objects.filter(visit_timestamp__date=yesterday).count()
    visitors_last_week = VisitorLog.objects.filter(visit_timestamp__date__gte=last_week).count()
    visitors_last_month = VisitorLog.objects.filter(visit_timestamp__date__gte=last_month).count()
    visitors_total = VisitorLog.objects.count()
    
    # Count unique visitors
    unique_visitors_today = VisitorLog.objects.filter(visit_timestamp__date=today).values('ip_address').distinct().count()
    unique_visitors_total = VisitorLog.objects.values('ip_address').distinct().count()
    
    # Most visited pages
    most_visited_pages = VisitorLog.objects.values('page_visited').annotate(
        count=Count('page_visited')).order_by('-count')[:5]
    
    # Visitor breakdown by browser
    browser_stats = VisitorLog.objects.values('browser').annotate(
        count=Count('browser')).order_by('-count')[:5]
    
    # Visitor breakdown by device
    device_stats = VisitorLog.objects.values('device').annotate(
        count=Count('device')).order_by('-count')[:3]
    
    # Visitor breakdown by country
    country_stats = VisitorLog.objects.exclude(country__isnull=True).values('country').annotate(
        count=Count('country')).order_by('-count')[:5]
...
@login_required
def update_money_donation_status(request, donation_id):
    """Update the status of a money donation from the admin dashboard."""
    if not request.user.is_superuser:
        messages.error(request, 'You do not have permission to update donation status.')
        return redirect('food_donation:home')
    ...
    if request.method == 'POST':
        try:
            ...
            # If status is approved, mark as acknowledged
            if status == 'approved':
                donation.is_acknowledged = True
            
            donation.save()
            
            # Send receipt if status changed to approved
            if status == 'approved':
                try:
                    from .utils import send_money_donation_confirmation
                    send_money_donation_confirmation(donation.donor, donation, is_receipt=True)
                    messages.success(request, f"Receipt sent to {donation.donor.email}.")
                except Exception as e:
                    messages.warning(request, f"Status updated but failed to send receipt: {str(e)}")
            ...
            messages.error(request, f"Error updating donation status: {str(e)}")
    
    return redirect('food_donation:admin_dashboard')

@login_required
def cancel_money_donation(request, donation_id):
    """Cancel a money donation from the admin dashboard."""
    if not request.user.is_superuser:
        messages.error(request, 'You do not have permission to cancel donations.')
        return redirect('food_donation:home')
    ...
    if request.method == 'POST':
        try:
            ...
            if not donation.can_cancel:
                messages.error(request, "This donation cannot be cancelled. Only pending and unacknowledged donations can be cancelled.")
                return redirect('food_donation:admin_dashboard')
            
            # Update donation status and add cancellation reason to notes
            cancellation_reason = request.POST.get('cancel_reason', '')
            if donation.notes:
                donation.notes += f"\n\nCANCELLATION REASON ({timezone.now().strftime('%Y-%m-%d %H:%M')}): {cancellation_reason}"
            else:
                donation.notes = f"CANCELLATION REASON ({timezone.now().strftime('%Y-%m-%d %H:%M')}): {cancellation_reason}"
            
            donation.status = 'cancelled'
            donation.save()
            
            # Notify the donor about the cancellation
            try:
                ...
                Notification.objects.create(
                    ...
                    message=f'Your donation of ₹{donation.amount} has been cancelled. Reason: {cancellation_reason}',
                )
                messages.success(request, f"Donation cancelled and donor notified.")
            except Exception as e:
                messages.warning(request, f"Donation cancelled but failed to notify donor: {str(e)}")
...
@login_required
def admin_money_donation_receipt(request, donation_id):
    """Generate and download a receipt for a money donation."""
    if not request.user.is_superuser:
        messages.error(request, 'You do not have permission to access this page.')
        return redirect('food_donation:home')
    ...
    try:
        ...
        # Generate receipt only for approved donations
        if donation.status != 'approved':
            messages.error(request, "Receipt is only available for approved donations.")
            return redirect('food_donation:admin_dashboard')
        ...
        context = {
            ...
            'is_receipt': True,
            'receipt_number': f"R-{donation.transaction_id or donation.id}",
            'current_date': timezone.now().strftime('%Y-%m-%d')
        }
        
        html_content = render_to_string('food_donation/email/money_donation_receipt.html', context)
...
# Bhandara (Free Food Distribution) Views
def bhandara_list(request):
    """Display a list of Bhandara events with map."""
    from django.utils import timezone
    from .models import Bhandara
    
    # Get active and approved Bhandara events
    bhandaras = Bhandara.objects.filter(
        is_approved=True, 
        is_active=True,
    ...
    return render(request, 'food_donation/bhandara.html', context)
...
def volunteer_signup(request, event_id):
    """Handle volunteer signup for a Bhandara event."""
    from django.utils import timezone
    from .models import Bhandara, Volunteer, UserProfile, Notification
    from django.shortcuts import get_object_or_404
    
    # Get the specific Bhandara event
    event = get_object_or_404(Bhandara, pk=event_id, is_approved=True, is_active=True)
    ...
    if request.method == 'POST':
        ...
        if not user:
            # Create a new user
            username = email.split('@')[0] + str(timezone.now().timestamp())[:5]
        ...
        # Create a notification for the event organizer if there's a user account
        if event.created_by:
            Notification.objects.create(
                user=event.created_by,
                notification_type='volunteer',
                title=f'New Volunteer for {event.name}',
                message=f'{name} has signed up to volunteer for your Bhandara event.',
                related_id=event.id
            )
        
        # Send email notifications (future implementation)
        
        messages.success(request, f'Thank you for volunteering for {event.name}! The organizer will contact you soon.')
        return redirect('food_donation:bhandara_detail', pk=event_id)
    
    # If not POST, redirect back to event detail
    return redirect('food_donation:bhandara_detail', pk=event_id)

def event_ical(request, event_id):
    """Generate iCal file for a Bhandara event."""
    from django.utils import timezone
    from .models import Bhandara
    from django.shortcuts import get_object_or_404
    import icalendar
    ...
    cal.add('prodid', '-//Beyond Hunger//beyoundhunger.org//')
    cal.add('version', '2.0')
    ...
    ical_event.add('location', f"{event.address}, {event.city}, {event.state}, {event.postal_code}")
    ...
    response['Content-Disposition'] = f'attachment; filename="{event.name.replace(" ", "_")}.ics"'
    return response

@login_required
def manage_bhandaras(request):
    """Admin view to manage Bhandara events."""
    if not request.user.is_superuser:
        messages.error(request, 'You do not have access to manage Bhandara events.')
        return redirect('food_donation:home')
    
    from .models import Bhandara
    
    # Get all Bhandara events
    pending_bhandaras = Bhandara.objects.filter(is_approved=False).order_by('-created_at')
    approved_bhandaras = Bhandara.objects.filter(is_approved=True).order_by('-created_at')
    
    # Process form submission
    if request.method == 'POST':
        if 'approve_bhandara' in request.POST:
            bhandara_id = request.POST.get('bhandara_id')
            try:
                bhandara = Bhandara.objects.get(id=bhandara_id)
                bhandara.is_approved = True
                bhandara.save()
                messages.success(request, f'Bhandara event "{bhandara.name}" has been approved.')
            except Bhandara.DoesNotExist:
                messages.error(request, 'Bhandara event not found.')
        
        elif 'reject_bhandara' in request.POST:
            bhandara_id = request.POST.get('bhandara_id')
            try:
                bhandara = Bhandara.objects.get(id=bhandara_id)
                bhandara.delete()
                messages.success(request, 'Bhandara event has been rejected and deleted.')
            except Bhandara.DoesNotExist:
                messages.error(request, 'Bhandara event not found.')
        
        elif 'deactivate_bhandara' in request.POST:
            bhandara_id = request.POST.get('bhandara_id')
            try:
                bhandara = Bhandara.objects.get(id=bhandara_id)
                bhandara.is_active = False
                bhandara.save()
                messages.success(request, f'Bhandara event "{bhandara.name}" has been deactivated.')
            except Bhandara.DoesNotExist:
                messages.error(request, 'Bhandara event not found.')
        
        elif 'activate_bhandara' in request.POST:
            bhandara_id = request.POST.get('bhandara_id')
            try:
                bhandara = Bhandara.objects.get(id=bhandara_id)
                bhandara.is_active = True
                bhandara.save()
                messages.success(request, f'Bhandara event "{bhandara.name}" has been activated.')
            except Bhandara.DoesNotExist:
                messages.error(request, 'Bhandara event not found.')
        
        # Redirect to avoid form resubmission
        return redirect('food_donation:manage_bhandaras')
    
    context = {
        'pending_bhandaras': pending_bhandaras,
        'approved_bhandaras': approved_bhandaras,
    }
    
    return render(request, 'food_donation/manage_bhandaras.html', context)

@login_required
def visitor_logs(request):
    """Admin view to display visitor logs."""
    if not request.user.is_superuser:
        messages.error(request, 'You do not have access to view visitor logs.')
        return redirect('food_donation:home')
    ...
    page_visited = request.GET.get('page_visited', '')
    ...
    # Apply filters
    if date_range == 'today':
        logs = logs.filter(visit_timestamp__date=now().date())
    elif date_range == 'yesterday':
        logs = logs.filter(visit_timestamp__date=now().date() - timedelta(days=1))
    elif date_range == 'week':
        logs = logs.filter(visit_timestamp__date__gte=now().date() - timedelta(days=7))
    elif date_range == 'month':
        logs = logs.filter(visit_timestamp__date__gte=now().date() - timedelta(days=30))
    
    if country:
        logs = logs.filter(country__icontains=country)
    
    if page_visited:
        logs = logs.filter(page_visited__icontains=page_visited)
    
    # Order by most recent
    logs = logs.order_by('-visit_timestamp')
    
    # Get statistics
    stats = {
        'total_visits': logs.count(),
        'unique_visitors': logs.values('ip_address').distinct().count(),
        'most_visited_pages': logs.values('page_visited').annotate(count=Count('page_visited')).order_by('-count')[:10],
        'browser_stats': logs.values('browser').annotate(count=Count('browser')).order_by('-count'),
        'device_stats': logs.values('device').annotate(count=Count('device')).order_by('-count'),
        'country_stats': logs.exclude(country__isnull=True).values('country').annotate(count=Count('country')).order_by('-count')[:10],
    }
    ...
    return render(request, 'food_donation/visitor_logs.html', context)

@login_required
def create_bhandara(request):
    """Create a new Bhandara event."""
    from django.utils import timezone
    from .models import Bhandara
    from .forms import BhandaraForm
    
    if request.method == 'POST':
        form = BhandaraForm(request.POST, request.FILES)
        if form.is_valid():
            bhandara = form.save(commit=False)
            bhandara.created_by = request.user
            bhandara.save()
            
            messages.success(request, 'Your Bhandara event has been submitted for approval.')
            return redirect('food_donation:bhandara')
    else:
        form = BhandaraForm()
    ...
    return render(request, 'food_donation/create_bhandara.html', context)

@login_required
@user_passes_test(lambda u: u.is_superuser)
def delete_food_donation(request, donation_id):
    """Delete a food donation and optionally notify the donor"""
    if request.method == 'POST':
        try:
            # Get the donation
            donation = FoodDonation.objects.get(id=donation_id)
            
            # Get details for potential email notification
            donor = donation.donor
            ...
            reason = request.POST.get('delete_reason', 'Administrative action')
            notify_donor = 'notify_donor' in request.POST
            
            # Send email notification if requested and donor has email
            if notify_donor and donor_email:
                try:
                    from django.core.mail import send_mail
                    from django.conf import settings
                    
                    subject = "Your food donation has been removed"
                    message = f"""Dear {donor.first_name or donor.username},

We regret to inform you that your food donation of "{food_type}" made on {donation_date} has been removed from our system.
...
"""
                    ...
                    messages.success(request, f"Notification email sent to {donor_email}")
                except Exception as e:
                    messages.warning(request, f"Error sending email to {donor_email}: {str(e)}")
            
            # Delete the donation
            donation.delete()
            messages.success(request, "Food donation deleted successfully.")
...
@login_required
@user_passes_test(lambda u: u.is_superuser)
def delete_money_donation(request, donation_id):
    """Delete a money donation and optionally notify the donor"""
    if request.method == 'POST':
        try:
            # Get the donation
            donation = MoneyDonation.objects.get(id=donation_id)
            ...
            if notify_donor and donor_email:
                try:
                    ...
                    subject = "Your donation has been removed"
                    message = f"""Dear {donor.first_name or donor.username},

We regret to inform you that your monetary donation of ₹{amount} made on {donation_date} has been removed from our system.
...
"""
            ...
            # Delete the donation
            donation.delete()
            messages.success(request, "Money donation deleted successfully.")
...
@login_required
def notify_payment(request, donation_id):
    """Allows users to update their donation with payment details"""
    try:
        donation = MoneyDonation.objects.get(id=donation_id, donor=request.user)
        ...
        if request.method == 'POST':
            ...
            try:
                admin_email = 'beyoundhunger1@gmail.com'
                subject = f"Payment Notification - Donation #{donation.id}"
                message = f"""
                A user has submitted payment details for their donation:
                
                Donation ID: {donation.id}
                Amount: ₹{donation.amount}
                Transaction ID: {transaction_id}
                Payment Method: {payment_method}
                PAN Number: {pan_number if pan_number else 'Not provided'}
                
                Please verify this payment and update the donation status in the admin panel.
                """
                from django.core.mail import send_mail
                send_mail(
                    subject=subject,
            ...
            messages.success(request, "Thank you! We've received your payment details and will verify your donation shortly.")
        
        return redirect('food_donation:money_donation_history')
    except MoneyDonation.DoesNotExist:
        messages.error(request, "Donation not found or you don't have permission to update it.")
        return redirect('food_donation:money_donation_history')
