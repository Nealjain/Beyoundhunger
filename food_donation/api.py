...
from .models import FoodDonation, DeliveryAssignment, Volunteer, UserProfile, ChatbotResponse, Bhandara
...
class BhandaraSerializer(serializers.ModelSerializer):
    google_maps_url = serializers.SerializerMethodField()
    is_ongoing = serializers.SerializerMethodField()
    start_datetime = serializers.DateTimeField(format="%b %d, %Y - %I:%M %p")
    end_datetime = serializers.DateTimeField(format="%b %d, %Y - %I:%M %p")
    
    class Meta:
        model = Bhandara
        fields = [
            'id', 'name', 'description', 'organizer_name', 'contact_phone',
            'address', 'city', 'state', 'postal_code', 'latitude', 'longitude',
            'start_datetime', 'end_datetime', 'food_items', 'is_recurring',
            'recurrence_pattern', 'is_approved', 'is_active', 'google_maps_url',
            'is_ongoing', 'image'
        ]
...
@api_view(['GET'])
def bhandara_detail_api(request, pk):
    """Get details for a specific Bhandara event"""
    try:
        bhandara = Bhandara.objects.get(pk=pk, is_approved=True, is_active=True)
        serializer = BhandaraSerializer(bhandara)
        return Response(serializer.data)
    except Bhandara.DoesNotExist:
        return Response({'error': 'Bhandara not found'}, status=status.HTTP_404_NOT_FOUND) 